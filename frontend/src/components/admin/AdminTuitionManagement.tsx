  ...
  const handleLoadStudents = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/admin/tuitions/department/${batchFormData.deptCode}/students`,
        {
          params: {
      ...
      <Card title="등록금 납부 관리">
        <div className="mb-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => setIsBatchModalOpen(true)}>과별 일괄 생성</Button>
                      ...
                      <thead className="bg-slate-100">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase">
                            <input
                              type="checkbox"
                              checked={selectedStudents.length === studentList.filter(s => !s.hasExistingTuition).length && studentList.filter(s => !s.hasExistingTuition).length > 0}
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {studentList.map((student) => (
                          <tr
                            ...
                            className={student.hasExistingTuition ? "bg-slate-50" : ""}
                          >
