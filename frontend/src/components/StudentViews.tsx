  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Modal isOpen={isModalOpen} onClose={() => !isProcessing && setIsModalOpen(false)} title="복학 신청 확인">
        <div className="space-y-4">
          <p className="text-slate-600">다음 학기에 복학을 신청하시겠습니까?</p>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Card title="복학 내역 조회">
        <div className="mb-6 flex justify-between items-center">
          ...
          <Button onClick={() => navigate("/student/return-application")}>복학 신청하기</Button>
        </div>
        ...
        {returnHistory.length === 0 ? (
          ...
          <div className="space-y-4">
            {returnHistory.map((record) => (
              <div key={record.id} className="border border-slate-200 rounded-lg p-5 hover:shadow-md transition-shadow bg-white">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-bold text-slate-800">
                        {record.returnYear}년 {record.returnSemester}학기 복학
                      </h3>
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${getStatusBadge(record.status)}`}>{record.status}</span>
                    </div>
                    <p className="text-sm text-slate-500">{record.leaveType}에서 복학</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-400">신청일</p>
                    <p className="text-sm font-medium text-slate-700">{record.applicationDate}</p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">승인일</span>
                    <span className="text-slate-800 font-medium">{record.approvalDate || "-"}</span>
                  </div>
                </div>
              </div>
            ))}
