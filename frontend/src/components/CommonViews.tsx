...
import type { User, AcademicSchedule } from "../types";
    ...
    <Card title="개인 정보">
      <div className="flex items-center space-x-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          ...
          <div>
            <span className="font-semibold text-slate-600">소속:</span> {user.departmentName ?? ""}
          </div>
          <div>
      ...
      {MOCK_ANNOUNCEMENTS.map((ann) => (
        <li key={ann.postId} className="py-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm text-slate-500">{ann.createdAt.substring(0, 10)}</span>
          </div>
      ...
      <div key={day} className="border-r border-b border-slate-200 p-2 min-h-[120px] flex flex-col relative">
        ...
        <div className="flex-grow space-y-1 mt-1 overflow-y-auto">
          {dayEvents.map((event) => {
            ...
            return (
              <div key={event.scheduleId} title={event.title} className={`text-xs p-1 rounded-md truncate ${colorClass}`}>
                {event.title}
              </div>
            );
          })}
...
export const AcademicCalendar: React.FC = () => {
  ...
  const eventsForYear = MOCK_CALENDAR_EVENTS.filter((event) => new Date(event.startDate).getFullYear() === selectedYear);
